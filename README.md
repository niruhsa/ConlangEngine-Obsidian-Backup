...
If you downloaded the `.zip` from the [releases tab](https://github.com/niruhsa/ConlangEngine-Obsidian-Backup/releases) you can skip this step — the release archive already contains a compiled `main.js`.

If you cloned the repository with `git`, you **must** build the plugin first. The compiled `main.js` is deliberately not committed to the repo, so a fresh clone will not load in Obsidian until you run a build.

You will need [Node.js](https://nodejs.org) 18 or newer (which includes `npm`). From inside the plugin directory you just cloned:

```bash
# Install the build dependencies
npm install

# Type-check and produce a production main.js
npm run build
```

When it finishes, the plugin directory should contain the three files Obsidian needs, sitting directly next to each other:

```
<vault path>/.obsidian/plugins/ConlangEngine-Obsidian-Backup/
├── main.js       # generated by `npm run build`
├── manifest.json
└── styles.css
```

If `main.js` is missing, the build did not complete — re-run `npm run build` and check the output for errors.

The available scripts are:

| Script | Purpose |
| --- | --- |
| `npm run build` | Type-checks the source, then bundles a minified production `main.js`. Use this for a normal install. |
| `npm run dev` | Starts esbuild in watch mode and rebuilds `main.js` on every save. Use this while developing. |
| `npm run typecheck` | Runs the TypeScript compiler in `--noEmit` mode without bundling. |
| `npm run lint` | Runs ESLint over the project. |

> **Note on TypeScript versions:** the type-check runs on TypeScript 7, which is installed under the aliased package name `typescript-7`. TypeScript 5 is also kept as a dependency because `typescript-eslint` does not support TypeScript 7 yet, and `npm run lint` still needs it. Both are installed automatically by `npm install` — you do not need to install TypeScript globally.

After building, restart Obsidian (or reload it with `Ctrl`/`Cmd` + `R`) so it picks up the new plugin folder.
